...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, attention_mask=None, attention_weights=None):
        ...
        aop_fea_flat = self.LN2(aop_fea_flat)
        q_ir = self.que_proj_ir(aop_fea_flat).contiguous().view(b_s, nq, self.h, self.d_k).permute(0, 2, 1, 3)  # (b_s, h, nq, d_k)
        k_ir = self.key_proj_ir(aop_fea_flat).contiguous().view(b_s, nk, self.h, self.d_k).permute(0, 2, 3, 1)  # (b_s, h, d_k, nk) K^T
        v_ir = self.val_proj_ir(aop_fea_flat).contiguous().view(b_s, nk, self.h, self.d_v).permute(0, 2, 1, 3)  # (b_s, h, nk, d_v)
...
class CrossTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        rgb_fea_flat = x[0]
        aop_fea_flat = x[1]
        assert rgb_fea_flat.shape[0] == aop_fea_flat.shape[0]
        bs, nx, c = rgb_fea_flat.size()
        h = w = int(math.sqrt(nx))

        for loop in range(self.loops):
            # with Learnable Coefficient
            rgb_fea_out, aop_fea_out = self.crossatt([rgb_fea_flat, aop_fea_flat])#QK
            
            rgb_att_out = self.coefficient1(rgb_fea_flat) + self.coefficient2(rgb_fea_out)
            aop_att_out = self.coefficient3(aop_fea_out) + self.coefficient4(aop_fea_out)
            
            rgb_fea_flat = self.coefficient5(rgb_att_out) + self.coefficient6(self.mlp_vis(self.LN2(rgb_att_out)))
            aop_fea_flat = self.coefficient7(aop_att_out) + self.coefficient8(self.mlp_ir(self.LN2(aop_att_out)))

        return [rgb_fea_flat, aop_fea_flat]
...
class TransformerFusionBlock(nn.Module):
    def __init__(self, d_model, vert_anchors=16, horz_anchors = 16, h=8, block_exp=4, n_layer=1, embd_pdrop=0.1, attn_pdrop=0.1, resid_pdrop=0.1):
        ...
        # positional embedding parameter (learnable), aop_fea + cop_fea
        self.pos_emb_cop = nn.Parameter(torch.zeros(1, vert_anchors * horz_anchors, self.n_embd))
        self.pos_emb_aop = nn.Parameter(torch.zeros(1, vert_anchors * horz_anchors, self.n_embd))
        ...
        self.cop_coefficient = LearnableWeights()
        self.aop_coefficient = LearnableWeights()
    ...
    def forward(self, x):
        ...
        #print(cop_fea.shape,self.vert_anchors,self.horz_anchors)

        assert cop_fea.shape[0] == aop_fea.shape[0]
        assert cop_fea.shape[2] == aop_fea.shape[2]
        assert cop_fea.shape[3] == aop_fea.shape[3]
        bs, c, h, w = cop_fea.shape

        # ------------------------- cross-modal feature fusion -----------------------#
        new_cop_fea = self.cop_coefficient(self.avgpool(cop_fea), self.maxpool(cop_fea))
        new_c, new_h, new_w = new_cop_fea.shape[1], new_cop_fea.shape[2], new_cop_fea.shape[3]
        cop_fea_flat = new_cop_fea.contiguous().view(bs, new_c, -1).permute(0, 2, 1) + self.pos_emb_cop

        new_aop_fea = self.aop_coefficient(self.avgpool(aop_fea), self.maxpool(aop_fea))
        aop_fea_flat = new_aop_fea.contiguous().view(bs, new_c, -1).permute(0, 2, 1) + self.pos_emb_aop

        cop_fea_flat, aop_fea_flat = self.crosstransformer([cop_fea_flat, aop_fea_flat])
        cop_fea_CFE = cop_fea_flat.contiguous().view(bs, new_h, new_w, new_c).permute(0, 3, 1, 2)
        
        if self.training == True:
            cop_fea_CFE = F.interpolate(cop_fea_CFE, size=([self.vert_anchors, self.horz_anchors]), mode='nearest')
        else:
            cop_fea_CFE = F.interpolate(cop_fea_CFE, size=([self.vert_anchors, self.horz_anchors]),mode='bilinear')
        new_cop_fea = cop_fea_CFE + new_cop_fea
        
        aop_fea_CFE = aop_fea_flat.contiguous().view(bs, new_h, new_w, new_c).permute(0, 3, 1, 2)
        if self.training == True:
            aop_fea_CFE = F.interpolate(aop_fea_CFE, size=([self.vert_anchors, self.horz_anchors]), mode='nearest')
        else:
            aop_fea_CFE = F.interpolate(aop_fea_CFE, size=([self.vert_anchors, self.horz_anchors]),mode='bilinear')
        
        new_aop_fea = aop_fea_CFE + new_aop_fea

        new_fea = self.concat([new_cop_fea, new_aop_fea])
        new_fea = self.conv1x1_out(new_fea)

        # ------------------------- feature visulization -----------------------#
        # save_dtop = '/data/lyh/feature_save/'
        # fea_cop = torch.mean(cop_fea, dim=1)
        # fea_cop_CFE = torch.mean(cop_fea_CFE, dim=1)
        # fea_cop_new = torch.mean(new_cop_fea, dim=1)
        # fea_top = torch.mean(top_fea, dim=1)
        # fea_top_CFE = torch.mean(top_fea_CFE, dim=1)
        # fea_top_new = torch.mean(new_top_fea, dim=1)
        # fea_new = torch.mean(new_fea, dim=1)
        # block = [fea_cop, fea_cop_CFE, fea_cop_new, fea_top, fea_top_CFE, fea_top_new, fea_new]
        # black_name = ['fea_cop', 'fea_cop After CFE', 'fea_cop skip', 'fea_top', 'fea_top After CFE', 'fea_top skip', 'fea_top NiNfusion']
        # plt.figure()
        ...
        #     plt.imshow(feature)
        # plt.savefig(save_dtop + 'fea_{}x{}.png'.format(h, w), dpi=300)
        # -----------------------------------------------------------------------------#

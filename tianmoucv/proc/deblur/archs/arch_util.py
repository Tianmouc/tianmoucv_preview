...
class Mutual_Attention(nn.Module):
    def __init__(self, dim, num_heads, bias):
        ...
        # # 保存路径
        # self.save_path = "attention_log.txt"
        # if os.path.exists(self.save_path):
        #     os.remove(self.save_path)  # 每次运行前清空旧的日志文件
    ...
    def forward(self, x, y):
        ...
        # # 写入完整 attention map 到 txt
        # 主网络encode做了3次卷积以及EICA，所以每推理一张图会得到3个attention map，按顺序写入txt
        # np.set_printoptions(threshold=np.inf)  # 不截断显示

        # attn_np = attn[0, 0].detach().cpu().numpy()
        # with open(self.save_path, "a") as f:
        #     f.write(f"Attention Map Shape: {attn_np.shape}\n")
        #     f.write("Attention Map Values (b=0, head=0):\n")
        #     f.write(np.array2string(attn_np, separator=', '))
        #     f.write("\n\n")
...
class EventImage_ConcatFusionBlock(nn.Module):
    def __init__(self, dim, ffn_expansion_factor=2, bias=False, LayerNorm_type='WithBias'):
        ...
        self.norm_image = LayerNorm(dim, LayerNorm_type)
    ...
    def forward(self, image, event):
        ...
        concat = torch.cat([image_norm, event_norm], dim=1)  # b, 2c, h, w

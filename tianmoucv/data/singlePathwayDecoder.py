...
def cone_tmdat_to_npy(condDataPath, idx=0, mode='RGB',print_info=True):
    '''
    @mode: RGB or RAW
    '''
    cone_width = 320
...
def rod_tmdat_to_npy(rodDataPath, idx=0,print_info=True):
    rod_width = 160
    rod_height = 160
...
def multiple_rods_tmdat_to_npy(rodDataPath, idx=0, length=1, print_info=True):
    rod_width = 160
    rod_height = 160
    temp_diff_np = np.zeros((length, rod_width * rod_height), dtype=np.int8)
    spat_diff_left_np = np.zeros((length, rod_width * rod_height), dtype=np.int8)
    spat_diff_right_np = np.zeros((length, rod_width * rod_height), dtype=np.int8)
    pkt_size_np = np.zeros([length], dtype=np.int32)
    pkt_size_td = np.zeros([length], dtype=np.int32)
    pkt_size_sd = np.zeros([length], dtype=np.int32) 
    rod_img_timestamp_np = np.zeros([length], dtype=np.uint64)
    rod_fcnt_np = np.zeros([length], dtype=np.int32)
    rod_adcprec_np = np.zeros([length], dtype=np.int32)        
    ...
    ret_code = rdc.get_multiple_rods_fullinfo(rodDataPath, rodAddrlist[idx],
                                                temp_diff_np, 
                                                spat_diff_left_np, 
                                                spat_diff_right_np,
                                                pkt_size_np, 
    ...
    tsd = np.zeros([3,length,rod_height,rod_width])
    tsd[0,:,:,:] = np.reshape(temp_diff_np.astype(np.float32),(length,rod_height,rod_width))
    tsd[1,:,:,:] = np.reshape(spat_diff_left_np.astype(np.float32),(length,rod_height,rod_width))
    tsd[2,:,:,:] = np.reshape(spat_diff_right_np.astype(np.float32),(length,rod_height,rod_width))

    rod_img_timestamp_np = [rod_img_timestamp_np[i].astype(np.int64) for i in range(rod_img_timestamp_np.shape[0])]
    
    return tsd,rod_img_timestamp_np




def multiple_rods_tmdat_to_npy_from_addr(rodDataPath, addr=0, length=1, print_info=True):
    rod_width = 160
    rod_height = 160
    temp_diff_np = np.zeros((length, rod_width * rod_height), dtype=np.int8)
    spat_diff_left_np = np.zeros((length, rod_width * rod_height), dtype=np.int8)
    ...
    ret_code = rdc.get_multiple_rods_fullinfo(rodDataPath, addr,
                                                temp_diff_np, 
                                                spat_diff_left_np, 
                                                spat_diff_right_np,
                                                pkt_size_np, 
